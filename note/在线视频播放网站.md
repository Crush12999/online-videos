...
思路：自定义一个权限注解，需要权限控制的接口上加。通过AOP获取角色权限列表以及对应接口禁止的角色权限，然后进行比对求交集，如果存在交集，说明当前用户不具备接口访问权限。

当项目之后拓展出更多的角色，可以引入组的概念。同一个角色组拥有相同的权限，就可以不用写禁止的角色了。
...
与接口权限控制相似，通过切面获取参数列表，再编写对应的参数数据规则。
